...
let allShifts = [];
let allResponses = [];
let currentSubmittingShift = null; // 今まさに提出しようとしているシフトのデータ
...
function renderDashboard() {
    ...
    // まだ回答してなくて、かつ締め切りが過ぎていないものをカウント
    ...
    if (pendingShifts.length === 0) {
        ...
        dashboardList.innerHTML = createShiftCardsHTML(pendingShifts.slice(0, 3)); // 最大3件
    }
}
...
function openSubmissionModal(shiftId) {
    ...
    document.getElementById('submission-comment').value = ''; // コメント欄を空に
...
// ==========================================
// 4. サーバーへ提出する機能（ボールを投げる）
// ==========================================
async function submitShiftData() {
    if (!currentSubmittingShift) return;
    if (!confirm('この内容で店長に提出しますか？')) return;

    // 全ての日付の「◯△✕」と「時間」のデータをかき集める
    const dailyResponses = [];
    currentSubmittingShift.dates.forEach((dateInfo, index) => {
        const btnGroup = document.getElementById(`btn-group-${index}`);
        
        let status = 'unavailable'; // デフォルトは✕
    ...
    const payload = {
        ...
        dailyResponses: dailyResponses, // 集めた日別データ！
